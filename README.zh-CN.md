...
[English](README.md) · [**下载 Windows 版本**](https://github.com/omegawork/codex-quota-bar/releases/latest/download/CodexQuotaBar-windows-x64.zip) · [反馈问题](https://github.com/omegawork/codex-quota-bar/issues) · [Windows 构建](https://github.com/omegawork/codex-quota-bar/actions/workflows/windows.yml)

**每周剩余额度，就显示在 Codex 顶部。**

Codex Quota Bar 在 Windows 版 Codex 的 **Help / 帮助** 右侧显示周额度。不用额外登录，也不用切换到另一个面板。悬停第一段查看个人重置时间；第二段显示实验性重置预测。

![真实 Codex 桌面窗口中的深色额度条](assets/hero-dark.png)

<details><summary>查看亮色主题</summary>

![亮色主题中的 Codex Quota Bar](assets/hero-light.png)

</details>

## 三步开始

1. [下载 Windows ZIP](https://github.com/omegawork/codex-quota-bar/releases/latest/download/CodexQuotaBar-windows-x64.zip)。
2. 完整解压到一个准备长期保留的目录。
3. 双击 `CodexQuotaBar.exe`：Codex 已打开就直接显示，未打开就自动启动并等待窗口。

需要 Windows 10/11 x64、.NET Framework 4.8，以及已登录的 Codex 桌面版。本工具是未签名的社区独立程序，不修改 Codex。运行前可在[发布页核对 SHA256](https://github.com/omegawork/codex-quota-bar/releases/latest)。

周额度来自本机 Codex app server 读取的当前账户数据。重置预测来自 [codex-reset.com](https://codex-reset.com/)，属于**实验性信息**；公开请求不发送账户 ID、额度、登录信息或 X 凭据。`100% reset` 仅表示来源存在有效预告，不代表额度已经重置，也不是 OpenAI 的保证。
...
- 双击程序即可在需要时启动 Codex；最后一个 Codex 窗口关闭后，工具及其读取进程退出。
...
如需专用的开始菜单入口，在解压目录打开 PowerShell，运行一次：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\create-shortcut.ps1
```

随后使用开始菜单中的 **Codex + Quota Bar** 快捷方式；也可以手动将该快捷方式固定到任务栏。脚本只为当前用户创建一个快捷方式，不需要管理员权限。上述执行策略只作用于这一次 PowerShell 进程。快捷方式不是必需的，直接双击 EXE 也能在需要时启动 Codex。
...
快捷方式指向解压目录，移动工具后请重新创建。本工具不设置开机启动，不安装常驻监视器。
...
- **更新后启动较慢**：Codex 未打开时，双击 EXE 会最多等待 120 秒让窗口出现；超过时限后可等 Codex 打开，再运行工具。
