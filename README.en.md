...
Show your weekly remaining quota and reset forecast beside **Help** in the Windows Codex desktop app. Hover over the quota to see your personal reset time.

**Weekly usage limit 11% left**　│　**20% reset**

**每周额度剩余 11%**　│　**20% reset**

The second segment uses `N% reset`: an active reset announcement appears as red `100% reset`; otherwise it shows the experimental 24-hour probability, such as `20% reset`. Hover over it for the announced time or window and whether the latest update is a manual banked reset or a direct reset. The `100%` label means an active announcement, not that quota has already reset or that OpenAI guarantees the outcome.
...
1. Download `codex-quota-bar-v1.1.9-windows-x64.zip` from [Releases](https://github.com/Useless-Craft/codex-quota-bar/releases/latest).
